...
import br.com.teste.twgerenciadortarefas.modelos.Tarefa;
...
public class TarefasController {
	...
	public ModelAndView inserir(@Valid Tarefa tarefa, BindingResult result, HttpServletRequest request) {
		...
		if (tarefa.getDataExpiracao() == null) {
			...
			if (tarefa.getDataExpiracao().before(new Date())) {
				result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data de expiração não pode ser menor que atual");
			}
		}
		if (result.hasErrors()) {
			mv.setViewName("tarefas/inserir");
	...
	public ModelAndView alterar(@Valid Tarefa tarefa, BindingResult result) {
		...
		if (tarefa.getDataExpiracao() == null) {
			...
			if (tarefa.getDataExpiracao().before(new Date())) {
				result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data expiração não pode ser menor que atual!");
			}
		}
